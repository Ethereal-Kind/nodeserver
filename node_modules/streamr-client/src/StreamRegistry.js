...
let StreamRegistry = StreamRegistry_1 = class StreamRegistry {
    ...
    searchStreams(term, permissionFilter) {
        ...
        return (0, GeneratorUtils_1.map)((0, GeneratorUtils_1.filter)((0, searchStreams_1.fetchSearchStreamsResultFromTheGraph)(term, permissionFilter, this.graphQLClient), (item) => {
            try {
                Stream_1.Stream.parsePropertiesFromMetadata(item.stream.metadata);
                return true;
            }
            catch (err) {
        ...
        }), (item) => this.parseStream((0, streamr_client_protocol_1.toStreamID)(item.stream.id), item.stream.metadata));
    }
    ...
    async setPermissions(...items) {
        ...
        await this.connectToStreamRegistryContract();
        const ethersOverrides = this.ethereum.getStreamRegistryOverrides();
        const txToSubmit = this.streamRegistryContract.setPermissionsMultipleStreans(streamIds, targets, chainPermissions, ethersOverrides);
        await (0, contract_1.waitForTx)(txToSubmit);
    }
