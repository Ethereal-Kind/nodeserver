    ...
    async decrypt(streamMessage) {
        ...
        try {
            const groupKey = await this.keyExchange.getGroupKey(streamMessage).catch((err) => {
                throw new Encryption_1.UnableToDecryptError(`Could not get GroupKey: ${streamMessage.groupKeyId} – ${err.stack}`, streamMessage);
            });
            if (!groupKey) {
            ...
            const clone = streamr_client_protocol_1.StreamMessage.deserialize(streamMessage.serialize());
            Encryption_1.default.decryptStreamMessage(clone, groupKey);
