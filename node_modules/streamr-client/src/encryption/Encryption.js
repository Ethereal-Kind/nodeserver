...
function getSubtle() {
    ...
    const subtle = typeof window !== 'undefined' ? (_a = window === null || window === void 0 ? void 0 : window.crypto) === null || _a === void 0 ? void 0 : _a.subtle : webcrypto.subtle;
    if (!subtle) {
...
class StreamMessageProcessingError extends streamr_client_protocol_1.StreamMessageError {
...
class InvalidGroupKeyError extends streamr_client_protocol_1.ValidationError {
...
class GroupKey {
    constructor(groupKeyId, groupKeyBufferOrHexString) {
        this.id = groupKeyId;
        if (!groupKeyId) {
            throw new InvalidGroupKeyError(`groupKeyId must not be falsey ${(0, log_1.inspect)(groupKeyId)}`);
        }
        if (!groupKeyBufferOrHexString) {
            throw new InvalidGroupKeyError(`groupKeyBufferOrHexString must not be falsey ${(0, log_1.inspect)(groupKeyBufferOrHexString)}`);
        }
    ...
    static validate(maybeGroupKey) {
        if (!maybeGroupKey) {
            throw new InvalidGroupKeyError(`value must be a ${this.name}: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
        if (!(maybeGroupKey instanceof this)) {
            throw new InvalidGroupKeyError(`value must be a ${this.name}: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
        if (!maybeGroupKey.id || typeof maybeGroupKey.id !== 'string') {
            throw new InvalidGroupKeyError(`${this.name} id must be a string: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
        if (maybeGroupKey.id.includes('---BEGIN')) {
            throw new InvalidGroupKeyError(`${this.name} public/private key is not a valid group key id: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
        if (!maybeGroupKey.data || !Buffer.isBuffer(maybeGroupKey.data)) {
            throw new InvalidGroupKeyError(`${this.name} data must be a Buffer: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
        if (!maybeGroupKey.hex || typeof maybeGroupKey.hex !== 'string') {
            throw new InvalidGroupKeyError(`${this.name} hex must be a string: ${(0, log_1.inspect)(maybeGroupKey)}`, maybeGroupKey);
        }
    ...
    static generate(id = (0, utils_1.uuid)('GroupKey')) {
...
class EncryptionUtilBase {
    ...
    static encryptStreamMessage(streamMessage, groupKey, nextGroupKey) {
        ...
        streamMessage.encryptionType = streamr_client_protocol_1.StreamMessage.ENCRYPTION_TYPES.AES;
        ...
        if (nextGroupKey) {
            GroupKey.validate(nextGroupKey);
            streamMessage.newGroupKey = new streamr_client_protocol_1.EncryptedGroupKey(nextGroupKey.id, this.encrypt(nextGroupKey.data, groupKey));
        }
    ...
    static decryptStreamMessage(streamMessage, groupKey) {
        ...
        /* eslint-disable no-param-reassign */
        try {
            streamMessage.encryptionType = streamr_client_protocol_1.StreamMessage.ENCRYPTION_TYPES.NONE;
