...
exports.toReadableStream = exports.callbackToPromise = exports.eventsWithArgsToArray = exports.eventsToArray = exports.wait = exports.runAndWaitForConditions = exports.waitForCondition = exports.runAndRaceEvents = exports.runAndWaitForEvents = exports.waitForEvent = exports.waitForStreamToEnd = void 0;
...
exports.waitForStreamToEnd = waitForStreamToEnd;
/**
...
// internal
const runAndWait = async (operations, waitedEvents, timeout, promiseFn) => {
    ...
    const promise = promiseFn(evs.map(([emitter, event]) => exports.waitForEvent(emitter, event, timeout)));
    ops.forEach((op) => { op(); });
    return promise;
};
/**
 ...
 * within timeout. Otherwise rejected.
 */
const runAndWaitForEvents = async (operations, waitedEvents, timeout = 5000) => {
    return runAndWait(operations, waitedEvents, timeout, Promise.all.bind(Promise));
};
exports.runAndWaitForEvents = runAndWaitForEvents;
/**
 ...
 * within timeout. Otherwise rejected.
 */
const runAndRaceEvents = async (operations, waitedEvents, timeout = 5000) => {
    return runAndWait(operations, waitedEvents, timeout, Promise.race.bind(Promise));
};
exports.runAndRaceEvents = runAndRaceEvents;
/**
 ...
 */
const waitForCondition = async (conditionFn, timeout = 5000, retryInterval = 100, onTimeoutContext) => {
    // create error beforehand to capture more usable stack
 ...
 * is reached with conditionFn never evaluating to true, rejects.
 */
const runAndWaitForConditions = async (operations, conditions, timeout = 5000, retryInterval = 100, onTimeoutContext) => {
    const ops = Array.isArray(operations) ? operations : [operations];
    const conds = Array.isArray(conditions) ? conditions : [conditions];
    const promise = Promise.all(conds.map((condition) => exports.waitForCondition(condition, timeout, retryInterval, onTimeoutContext)));
    ops.forEach((op) => { op(); });
    return promise;
};
exports.runAndWaitForConditions = runAndWaitForConditions;
/**
...
exports.eventsWithArgsToArray = eventsWithArgsToArray;
/**
